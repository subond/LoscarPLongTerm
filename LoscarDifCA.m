...
[co2(k),pco2(k),co3(k),ph(k),kh(k),o2(k)] = ...
   dafunPECA(c(k)/rho,a(k)/rho,TX(k),Sv(k),Pv(k),CA(k),Mg);
end;
else
  for k=1:Nb
[co2(k),pco2(k),co3(k),ph(k),kh(k),o2(k)] = ...
...
for k=1:Ns
[kspcSed(k),x] = ...
     kspfunCA(Tdv(k),Sdv(k),zsatv(k)/10.,CA(k),Mg);
 kspcSed(k);
...
for k=kkv
[kspcS(k),kspaS(k)] = ...
     kspfunCA(TX(k),Sv(k),Pv(k),CA(k),Mg);
end;
...
CAp(k+3) = CAp(k+3)+eI*ECALv(k)/V(k+3);
%CHCKbioI(kt) = eI*ECALv(k)/V(k+3);
CAp(k+6) = CAp(k+6)+oI*ECALv(k)/V(k+6)...
                +   nu*ECALv(k)/V(k+6)/2;                 % D ClmnDiss
%CHCKbioD(kt) = oI*ECALv(k)/V(k+6) +nu*ECALv(k)/V(k+6)/2;
end;    
CAp(10)  =0;% CAp(10)  - ECAH/V10;
for k=7:9                                              % DA,DI,DP
CAp(k )  = CAp(k )  +ECAH/V(k)*gp(k)...    
                  +nu*ECAH/V(k)                *gp(k)/2;  %   
end;
if(ftys)
k = 11;
CAp(k  ) = CAp(k  )-      ECALv(4)/V(k  );
CAp(k+1) = CAp(k+1)+eI*ECALv(4)/V(k+1);
CAp(k+2) = CAp(k+2)+oI*ECALv(4)/V(k+2)...
                 +   nu*ECALv(4)/V(k+2);
end;
% riverine & sediment fluxes
if(fsed)                                           % #!
