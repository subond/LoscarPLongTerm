...
CBl = 3000.e15; % Blast       2200  2000  3000 n2500
     %CBl=1000.e15*g
nSi   = 0.7;          % 0.2 0.3
nCC   = 0.7;                % 0.4 0.3 1.0 0.5
plotflag = 1; % plot results
CAvflag  = 2; % 2:calcium is a variable and changes,1:calcium constant 
savf     = 0; % save end state
loadf    = 1; % load initial steady-state

BlFlag   = 2; % Blast 1: shot0, 2: cont release  

fcon     = 3; % 1: NADW, 2: NPDW, 3: SO
swcon    = 1; % 1: SO -> NP -> SO switch
...
for k=1:Ns
[kspc(k),x] = ...
     kspfunCA(Tdv(k),Sdv(k),zsatv(k)/10.,CALCv,Mg);
end;
...
%     load dat\B1D3BL0\YPE10Sed;
 ...
 load dat\PETMCa\YPE10SedCAv1.DAT %for PETM set-up but Ca=20.00
...
tic % start clock
[tv,Y] = myode15s('LoscarDif10',[t0 tfinal],Y0',options); % 23t 15s
toc % stop  clock
lt = length(tv);
...
if(savf == 1 && CAvflag == 0)  
    ...
    save                    dat\PETMCa\YPE10Sed.DAT YPE10Sed -ASCII -DOUBLE -TABS;
end
    else
if(savf == 1 && CAvflag == 0)  
    YPE10SedM = Y(lt,:)';
...
Ma  = sum(Y(lt,   Nb+1:2*Nb).*V)/Voc;    % TA inventory/V
Mp  = sum(Y(lt, 2*Nb+1:3*Nb).*V)/Voc;    % TA inventory/V
MCAe  = sum(Y(lt, 3*Nb+1:4*Nb).*V)/Voc;
Mcc = sum(Y(lt, 4*Nb+1:5*Nb).*V)/Voc ... % TC inventory/V
         +Y(lt,NO*Nb+2     )*Aoc/Voc;    % atmosphere
 DX0 = [Mc-Mci Ma-Mai Mp-Mpi Mcc-Mcci MCAe-MCAi];
DX0    
end;
...
%total Ca inventory ocean     (Pg C)
MCAL=sum(Y(lt, 1:Nb).*V)    *40/1e15;
...
S  = Sv(k);
P  = Pv(k);          
[co2,pco2,co3,ph,kh,o2] = dafunPE(dic,alk,TC,S,P,Cal,Mg);
co3tv(i,k)  = co3;
 phtv(i,k)  = ph;
end;
end;
...
for k=kkv
[kspcS,kspaS] = ...
     kspfun(TCvt(i,k),Sv(k),Pv(k),Cal,Mg);
omegCSvt(i,l) = co3tv(i,k)*Cal./kspcS;
omegASvt(i,l) = co3tv(i,k)*Cal./kspaS;
l = l + 1;
end
end
else
...
for k=kkv
[kspcS,kspaS] = ...
     kspfunCA(TCvt(i,k),Sv(k),Pv(k),CA(i,k),Mg);
omegCSvt(i,l) = co3tv(i,k)*CA(i,k)/kspcS;
omegASvt(i,l) = co3tv(i,k)*CA(i,k)/kspaS;
l = l + 1;
end
end
end;%CAvflag
...
hold off
ylabel('F_C, F_S_i inc. (mol C m^2 y^{-1})')
set(gca,'FontSize',fs);
set(gca,'XLim',axx);
set(gca,'XTickLabel',[])
