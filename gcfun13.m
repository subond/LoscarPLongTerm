...
%     flak=(interp1([1:5:66],fla_k,p,'pchip'));
    ...
    yc=(fmc+fwg+fmg+faom-fmeth)*(dbck-ac)-(interp1(1:length(dcv),dcv,p))*fmc-dg(1)*(fwg+fmg)-da*faom+dm*fmeth;
...
%     acv(1)=c13kurtz(3,1);
    fbc=(dc(1)*fwc+(interp1(1:length(dcv),dcv,p))*fmc+dg(1)*(fwg+fmg)-(fwc+fmc+fwg+fmg+faom-fmeth)*(dbck-ac)+da*faom-dm*fmeth)/ac;
    fbg=fwc+fmc+fwg+fmg+faom-fbc-fmeth;
