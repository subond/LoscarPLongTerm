...
%
%-------------------------------------
function yp = LoscarDifdCA101(t,y)
...
global Aoc rho Nb V A TH0 TH TS mv mhd kasv kkv TCv Sv Pv gp tA tI ...
    ...
    frkc fekc fdkc flakc ACT RUN gamma ws fGG pco20 pflag burCv FiN0 LTflag Rkg Fkgs...
    acv ybbv ybv ycv runtime FERT epspv;     
...
%      kspfunCA(Tdv(k),Sdv(k),zsatv(k)/10.,avCALC*1e-3,Mg);
        [kspcA(k),tmp] = ...
            kspfunCA(TdvA(k),SdvA(k),zsatv(k)/10.,avCALC*1e-3,Mg);
        [kspcI(k),tmp] = ...
             kspfunCA(TdvI(k),SdvI(k),zsatv(k)/10.,avCALC*1e-3,Mg);
        [kspcP(k),tmp] = ...
             kspfunCA(TdvP(k),SdvP(k),zsatv(k)/10.,avCALC*1e-3,Mg);
        if(ftys)
            [kspcT(k),tmp] = ...
                kspfunCA(TdvT(k),SdvT(k),zsatv(k)/10.,avCALC*1e-3,Mg);
        end 
end;
% co3satv = kspcSed./(avCALC*1e-3);
co3satvA  = kspcA./(avCALC*1e-3);
...
if(geoflag)
   if(LTflag)
      ...
      Fkg   =   ((interp1(1:length(fbgv),fbgv,tgc-kgct))*1.e12)/Aoc;        % mol C    /m2/y
      Fkgs  =   (((interp1(1:length(fwgv),fwgv,tgc-kgct))+(interp1(1:length(fmgv),fmgv,tgc-kgct)))*1.e12)/Aoc;
      FiN0   =   ((interp1(1:length(fwcv),fwcv,tgc-kgct))*1.e12)/Aoc;
...
%       fbb=((interp1(1:length(ccv),ccv,tgc-kgct))+gamma(tgc)*0.087*log(pco2a/pco20)+0.087*(interp1(1:length(fGG),fGG,tgc-kgct)))*(2*(pco2a/pco20)/(1+(pco2a/pco20)))^FERT;
      FSi0   = 5.0*1e12/Aoc*(fb+0.0000)*(interp1(1:length(frkc),frkc,(tgc-(kgct))))*(interp1(1:length(fekc),fekc,(tgc-(kgct))))*((interp1(1:length(fdkc),fdkc,(tgc-(kgct))))^0.65);
   end
...
if(fsed)
if(ftys)    
yp(1:4*Nb+2+8*Ns) = 0; % not sure about 12*Ns part, it was 8
else 
yp(1:4*Nb+2+6*Ns) = 0;
end;    
EXLv   = EPLv;
...
% if(fsed)                                        %   #!
% for k=1:3
% cacp(k  ) = cacp(k  ) + 2*Fin13*Aoc/V(k)/nOC         ... % Fin  L wthr:2
%                     + 2*FSi13*Aoc/V(k)/nOC         ... % #! Si
%                     - 1*Fkg  *Aoc/V(k)/nOC*alp*Rb(k);  % #! krgn          
%              
