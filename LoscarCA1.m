...
CAvflag  = 1; % 1:calcium is a variable and changes,0:calcium constant 
...
%     load dat\B1D3BL0\YPE10Sed;
  ...
  load YPE10SedCAv1.DAT
  elseif(bath == 2 && CAvflag == 0)
     load YPE10SedCA1.DAT 
...
tic % start clock
[tv,Y] = myode15s('LoscarDifCA1',[t0 tfinal],Y0',options); % 23t 15s
toc % stop  clock
lt = length(tv);
...
if (savf == 1 && CAvflag == 1)
    ...
    save                    YPE10SedCAv1.DAT  YPE10SedCAv1 -ASCII -DOUBLE -TABS;
elseif (savf == 1 && CAvflag == 0)
	YPE10SedCA1 = Y(lt,:)';
    
    save                    YPE10SedCA1.DAT  YPE10SedCA1 -ASCII -DOUBLE -TABS;
...
elseif(satflg == 2)
    ...
    CALC=sum(CAr);
for k=1:lzv
[x,x,x,x,x,x,kspc(k),x] = dafunPECA(1e-3,1e-3,2,Soc,zv(k)/10.,CALC,Mg);
%co3szv = kspc(k)/Cal(k);
end;
co3szv = kspc./CALC;
end;
...
calcium = CA;
save                    calcium.DAT  calcium -ASCII -DOUBLE -TABS;
%=====================================================%
%=================== plot results ====================%
%=====================================================%
if(~plotflag)
