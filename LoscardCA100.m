...
addpath('C:\Users\Nemanja\Desktop\Loscar d44Ca\myode');
...
epspca   = -1.3;    % -1.3 fractionation between seawater and sediments
if(ftys)
epspca   = -1.3;    % -1.3 fractionation between seawater and sediments
end;
d44CAin = -1.3;    % d13C of riverine flux 3.0 2.0 2.6
...
if(fdox) % fdox
if(CAvflag == 0)
    Y0 = [c0 a0 p0 dox0 cca0 cc0 C0 CC0]';
else
...
f44ca0A = Y0(5*Nb+3+6*Ns:5*Nb+2+7*Ns);
f44ca0I = Y0(5*Nb+3+7*Ns:5*Nb+2+8*Ns);
f44ca0P = Y0(5*Nb+3+8*Ns:5*Nb+2+9*Ns);
if(ftys)
fc0T   =  Y0(5*Nb+3+9*Ns:5*Nb+2+10*Ns);    
f13c0T =  Y0(5*Nb+3+10*Ns:5*Nb+2+11*Ns);
f44ca0T = Y0(5*Nb+3+11*Ns:5*Nb+2+12*Ns);
end    
end % fdox   
% calc initial phi
if(isempty(phic)) % phi = phi(fc)
...
FFca    = (phi1-phi0)/(1-phi1);
phiiAca = (phi0+FFca*fca0A)./(1+FFca*fca0A); 
phiiIca = (phi0+FcaF*fca0I)./(1+FFca*fca0I); 
phiiPca = (phi0+FFca*fca0P)./(1+FFca*fca0P); 
if(ftys) % ftys
phiiT = (phi0+FF*fc0T)./(1+FF*fc0T); 
phiiTca = (phi0+FFca*fca0T)./(1+FFca*fca0T); 
...
tic % start clock
[tv,Y] = myode15s('LoscarDifdCA100',[t0 tfinal],Y0',options); % 23t 15s
toc % stop  clock
lt = length(tv);
...
dYdt = ones(size(Y'));
for i=1:lt
dYdt(:,i) = LoscarDifdCA100(tv(i),Y(i,:)');
end;
end;
...
f44caA (:,l) = Y( :,l+NO*Nb+2+6*Ns);
f44caI (:,l) = Y( :,l+NO*Nb+2+7*Ns);
f44caP (:,l) = Y( :,l+NO*Nb+2+8*Ns);
m13calA(:,l) = mcalA( :,l).*f13cA( :,l)./fcA( :,l);
m13calI(:,l) = mcalI( :,l).*f13cI( :,l)./fcI( :,l);
m13calP(:,l) = mcalP( :,l).*f13cP( :,l)./fcP( :,l);
m44calA(:,l) = mcalA( :,l).*f44caA( :,l)./fcA( :,l);
m44calI(:,l) = mcalI( :,l).*f44caI( :,l)./fcI( :,l);
m44calP(:,l) = mcalP( :,l).*f44caP( :,l)./fcP( :,l);
...
M44calvA( l) = mcalA(lt,l).*f44caA(lt,l)./fcA(lt,l);
M44calvI( l) = mcalI(lt,l).*f44caI(lt,l)./fcI(lt,l);
M44calvP( l) = mcalP(lt,l).*f44caP(lt,l)./fcP(lt,l);
if(ftys)
fcT    (:,l) = Y( :,l+NO*Nb+2+9*Ns);  %    
f13cT  (:,l) = Y( :,l+NO*Nb+2+10*Ns);  %  
f44caT  (:,l) = Y( :,l+NO*Nb+2+11*Ns);
mcalT  (:,l) = Y( :,l+NO*Nb+2+9*Ns).*(rhos*(1-phivtT(:, l)));
McalvT (  l) = Y(lt,l+NO*Nb+2+9*Ns).*(rhos*(1-phivtT(lt,l)));
m13calT(:,l) = mcalT( :,l).*f13cT( :,l)./fcT( :,l);
m44calT(:,l) = mcalT( :,l).*f44caT( :,l)./fcT( :,l);
M13calvT( l) = mcalT(lt,l).*f13cT(lt,l)./fcT(lt,l);
M44calvT( l) = mcalT(lt,l).*f44caT(lt,l)./fcT(lt,l);
end;    
end;
...
RsAfca = (f44cafA./fcfA/Rstca-1)*1e3;
RsIfca = (f44cafI./fcfI/Rstca-1)*1e3;
RsPfca = (f44cafP./fcfP/Rstca-1)*1e3;
if(ftys)
...
RsTfca = (f44cafT./fcfT/Rstca-1)*1e3;
end;
...
a  (:,k)   = Y(:,  Nb+k)/(1e-3*rho); % mol/m3 -> mmol/kg
p  (:,k)   = Y(:,2*Nb+k)/(1e-3*rho); % mol/m3 -> mmol/kg
cac (:,k)   = Y(:,3*Nb+k)/(1e-3*rho); % mol/m3 -> mmol/kg
