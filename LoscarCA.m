...
global Aoc rho Nb V A TH0 TH TS mv mhd kasv kkv TCv Sv Pv gp tA tI ...
    ...
    CHKcarB1 CHKcarB2 CHKcarB3 CHKcar1 CHKcar2 CHKcar3 CHKcar4 wcvtA wcvtI wcvtP wcvtT;  
...
CAvflag  = 1; % 1:calcium is a variable,0:calcium constant 
...
p0   = p0*0.87;
CA0  = [10.30*onV];      % FAKE VARIABLE JUST A TRY
%p0   = [1:1:10]*.25*1e-3;
...
tic % start clock
[tv,Y] = myode15s('LoscarDifCA',[t0 tfinal],Y0',options); % 23t 15s
toc % stop  clock
lt = length(tv);
...
if (savf == 1 && CAvflag == 1)
    ...
    save                    YPE10SedCAv.DAT  YPE10SedCAv -ASCII -DOUBLE -TABS;
elseif (savf == 1 && CAvflag == 0)
	YPE10SedCA = Y(lt,:)';
    
    save                    YPE10SedCA.DAT  YPE10SedCA -ASCII -DOUBLE -TABS;
...
dic = c(lt,k)*1e-3;
alk = a(lt,k)*1e-3;
Ca  = CA(lt,k)*1e-3;%%%THIS ADDED !!!!!!!!!!!!!!
TC = TCvt(lt,k);
S  = Sv(k);
P  = Pv(k);          
[co2,pco2,co3,ph,kh,o2] = dafunPECA(dic,alk,TC,S,P,Ca,Mg);
khv(k)   = kh;
co2v(k)  = co2;
co3v(k)  = co3;
phv(k)   = ph;
...
for k=kkv
[kspcS,kspaS] = ...
     kspfunCA(TCvt(i,k),Sv(k),Pv(k),Ca,Mg);
...
elseif(satflg == 2)
for k=1:lzv
[x,x,x,x,x,x,kspc(k),x] = dafunPECA(1e-3,1e-3,2,Soc,zv(k)/10.,Ca,Mg);
end;
co3szv = kspc/Ca;
end;
