...
%
%-------------------------------------
function yp = LoscarDifdCAPerm1(t,y)
...
if myflag == 01;
    ...
    global Aoc rho Nb V A TH0 TH TS mv mhd kasv kkv TCv Sv Pv gp tA tI ...
        ...
        acv ybbv ybv ycv runtime FERT epspv wcvtA wcvtI wcvtP wcvtT epspcaV inorgF epspF biopF Fpw0...
        ...
        rREG0 rREGv REDPCv REDPC0 meanSpco2v EPLv0 dbc alphagc;
...
%             sum(FprvA)
%             sum(FprvA.*asvA*A(1))
%              sum(FprvA.*asvA*A(1))+ sum(FprvI.*asvI*A(2))+sum(FprvP.*asvP*A(3))+sum(FprvT.*asvT*A(11))
% %              sum(EPLv)
%              FprvA(9)*asvA(9)*A(1)
%              return
    % CA(k);
    ...
    if(geoflag)
        if(LTflag)
            ...
            fekci = fekc;
            fdkci = fdkc;
            aa=exp(-ACT*ws*(tgc-kgct)/570)*exp(ACT*fGG);
            ...
            FSigc   = 5*1e12/Aoc*(fb+0.0000)*frkci*fekci*(fdkci^0.65);
...
%           Ffep = Ffep0*doxMeanDeep/O0;
        ...
        if(Pscenario==7)
            po4bf0=0.01*0.868339464745500;%*0.868339464745500
            ocbf0 = 0.02*0.868339464745500; %*0.868339464745500
        end 
        if(Pscenario==9)
            po4bf0=0.005;
            ocbf0 = 0.04;
        ...
        if(Floegel)
            ...
            po4bf=po4bf0*(0.25+0.75*doxMeanDeep/O0);%*1/8^((TCv(9)-TCv0(9))/10); % fraction of PO4 rain that gets burried
...
%             return
    ...
    %=================== TCO2 ======================%
    % TH & mixing
    % sum(EPLv) % organic carbon mol/y
    % sum(ECLv) % total carbon EPLv+EALv/2;
    % sum(EXLv) % organic carbon = EPLv
    % sum(EALv) % inorganic carbon
%     sum(EPLv)+ sum(EPH)
%     sum(EALv)/2-sum(EALv)/2*nu
%     return
    ...
    if(fdox)
        ...
        doxp(10)  = doxp(10 ) +    (eI+oI)*ECH/V10       *REDO2C;       % H
        if(ftys)
            k = 11;
