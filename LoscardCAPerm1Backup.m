...
if myflag  == 01;
    if solflag == 01;
        ...
        inorgF   = 1; % inorganic CaCO3 precip. should be 1 when runninig Permian
        % for any other time period should be 0
        
        epspF = 1;   %Ca fractionation depends on CO32-;
        biopF = 1;      %increases intial bio pump
        ...
        dYflag = 0;
        dY0dt  = LoscarDifdCAPerm1backup(t0,Y0);
        ...
        tic % start clock
        [tv,Y] = myode15s('LoscarDifdCAPerm1backup',[t0 tfinal],Y0',options); % 23t 15s
        toc % stop  clock
        lt = length(tv);
        ...
        if(ftys == 1)
            if(bath == 4)
                ...
                if (savf == 1 && CAvflag == 2 && LTflag && inorgF)
                    ...
                    save                    dat/PERM44Ca/YPE10SedCAv1LT.DAT  YPE10SedCAv1LT -ASCII -DOUBLE -TABS;
                end
    ...
    if(dYflag == 1)
        ...
        dYdt = ones(size(Y'));
        for i=1:lt
            dYdt(:,i) = LoscarDifdCAPerm1backup(tv(i),Y(i,:)');
        end;
    end;
