...
% get the d13c curve. Next, you take the org C value and rewrite the model
% so that d13C is now predicted. If everything done correctly, this should
% give us a d13C curve identical to the original one

% 2) Now choose a starting year (60 or 50Ma) and set the fluxes in LOSCAR
% to be exactly the same to the ones in the given year in GEOCARB so that
% both models have the same steady-states.
 
% 3) Run GEOCARB for a million years and supply the data to LOSCAR. LOSCAR
% will calculate a new org C burial, feed this burial to GEOCARB and
% run it for a million year period and calculate all other fluxes and
% supply them to LOSCAR. Keep repeating this for the duration of simulation

% 4) Because the run is going to be over a long time period need to include
...
% predicted in the model right the predicted Ca doesn't match the
...
if myflag  == 01;
    if solflag == 01;
        ...
        global Aoc rho Nb V A TH0 TH TS mv mhd kasv kkv TCv Sv Pv gp tA tI ...
            ...
            Floegel rREGv REDPCv meanSpco2v;
        ...
        LTflag = 0; % Flag to switch Long-Term, LOSCAR-GEOCARB run
        ...
        Pscenario = 1; % 0 - default Phosphate weathering scenario, po4bf 0.5%
        ...
        Fopb0 = 1.5e10/Aoc; % 2.0e10 organic P burial
        Ffep0 = 0.6e10/Aoc; % 0.6e10 iron-sorbed P burial
        Fcap0 = 1.5e10/Aoc; % 1.5e10 calcium-bound P burial
        ...
        if(ftys)
            ...
            if(LTflag)
                Focb0   =   (fbgv(tgc)*1.e12)/Aoc;         % mol C    /m2/y 05
                Focw0  =((fwgv(tgc)+fmgv(tgc))*1.e12)/Aoc;
            end
        end
        ...
        if(ftys == 1)
            if (loadf == 1)
                ...
                elseif(bath == 2 && CAvflag == 2 && ~LTflag && ~inorgF)
                    if(Pfeed)
%                         if(Pscenario == 0)
%                             load dat/PETM44CaP/Psc0/YPE10SedCa.DAT
%                         end
%                         if(Pscenario == 1)
%                             load dat/PETM44CaP/Psc1/YPE10SedCa.DAT
%                         end
%                         if(Pscenario == 2)
%                             load dat/PETM44CaP/Psc2/YPE10SedCa.DAT
%                         end
%                         if(Pscenario == 3)
%                             load dat/PETM44CaP/Psc3/YPE10SedCa.DAT
%                         end
%                         if(Pscenario == 4)
%                             load dat/PETM44CaP/Psc4/YPE10SedCa.DAT
%                         end
%                         if(Floegel)
...
%                         end
        ...
        if(ftys)
            ...
            O0=(O0s(:,7).*V(7)+O0s(:,8).*V(8)+O0s(:,9)*V(9))./((V(7)+V(8)+V(9)));
        end
        % initial inventory
        if(CAvflag == 0)
        ...
        dYflag = 0;
        dY0dt  = LoscarDifdCAPerm1BU(t0,Y0);
        ...
        tic % start clock
        [tv,Y] = myode15s('LoscarDifdCAPerm1BU',[t0 tfinal],Y0',options); % 23t 15s
        toc % stop  clock
        lt = length(tv);
...
%                         end
    ...
    if(dYflag == 1)
        ...
        dYdt = ones(size(Y'));
        for i=1:lt
            dYdt(:,i) = LoscarDifdCAPerm1BU(tv(i),Y(i,:)');
        end;
    end;
