...
%
%-------------------------------------
function yp = LoscarDifdCA100(t,y)
...
f44cavA= y(6*Nb+3+6*Ns:6*Nb+2+7*Ns); % %calc 
f44cavI= y(6*Nb+3+7*Ns:6*Nb+2+8*Ns); % %calc 
f44cavP= y(6*Nb+3+8*Ns:6*Nb+2+9*Ns); % %calc
if(ftys)
fcvT  = y(6*Nb+3+9*Ns:6*Nb+2+10*Ns); % %calc
f13cvT= y(6*Nb+3+10*Ns:6*Nb+2+11*Ns); % %calc
f44cavT= y(6*Nb+3+11*Ns:6*Nb+2+12*Ns); % %calc
end;
end;
...
a    = y(  Nb+1:2*Nb     ); % TA      (mol/m3)
p    = y(2*Nb+1:3*Nb     ); % PO4     (mol/m3)
cac   = y(3*Nb+1:4*Nb     ); % T13CO2  (mol/m3)
...
cac   = y(5*Nb+1:6*Nb     ); % T13CO2  (mol/m3)
...
EAACA  = ECALv(1)+ECAH*gp(7); % +EAH (no H seds, see below)
EAICA   = ECALv(2)+ECAH*gp(8);
EAPCA   = ECALv(3)+ECAH*gp(9);
...
FprAca  = (1-nu)*EAACA/A(1); % mol C/m2/y
FprIca  = (1-nu)*EAICA/A(2); % mol C/m2/y
FprPca  = (1-nu)*EAPCA/A(3); % mol C/m2/y
%====== Carbon-13
...
Fpr44A  = (1-nu)*EAACAC/A(1); % mol C/m2/y
Fpr44I  = (1-nu)*EAICAC/A(2); % mol C/m2/y
Fpr44P  = (1-nu)*EAPCAC/A(3); % mol C/m2/y
...
fdpvca(2)= ( FIIca ...
   -fsh*sum(FprvIca(1:jj   ).*asvI(1:jj   )*A(2)) ) ...
  ./    sum(FprvIca(jj+1:Ns).*asvI(jj+1:Ns)*A(2))  ;
fdpvca(3)= ( FPPca ...
   -fsh*sum(FprvPca(1:jj   ).*asvP(1:jj   )*A(3)) ) ...
  ./    sum(FprvPca(jj+1:Ns).*asvP(jj+1:Ns)*A(3))  ;

fshvAca = [fsh*ones(1,jj) fdpvca(1)*ones(1,Ns-jj)];
fshvIca = [fsh*ones(1,jj) fdpvca(2)*ones(1,Ns-jj)];
fshvPca = [fsh*ones(1,jj) fdpvca(3)*ones(1,Ns-jj)];
...
FprTca   = (1-nu)*ECAT  /A(11); % mol C/m2/y
Fpr13T = (1-nu)*EATCC/2/A(11); % mol C/m2/y
Fpr44T = (1-nu)*EATCAC/A(11); % mol C/m2/y
...
fshvTca  = [fsh*ones(1,jj) fdpvca(4)*ones(1,Ns-jj)];
...
[kspcSed(k),x] = ...
     kspfunCA(Tdv(k),Sdv(k),zsatv(k)/10.,avCALC*1e-3,Mg);
 %kspcSed(k);
 %kspCHCK(kt)=kspcSed(k);
 %co3satv = kspcSed(k)/(CALC);
end;
...
if(fsed)
if(ftys)    
yp(1:5*Nb+2+12*Ns) = 0; % not sure about 12*Ns part, it was 8
else 
yp(1:5*Nb+2+6*Ns) = 0;
end;    
EXLv   = EPLv;
...
ccp(k  ) = ccp(k  ) + 2*Fin13*Aoc/V(k)/nOC         ... % Fin  L wthr:2
   ...
   CHKcar1(kt) = 2*Fin13*Aoc/V(k)/nOC         ... % 
                   + 2*FSi13*Aoc/V(k)/nOC         ... 
                   - 1*Fkg*Aoc/V(k)/nOC*alp*Rb(k);                    
             
ccp(k  ) = ccp(k  ) + sum(dissC13v(k,1   :n1))/V(k  ); % diss L
CHKcar2(kt) =sum(dissC13v(k,1   :n1))/V(k  );
ccp(k+3) = ccp(k+3) + sum(dissC13v(k,n1+1:n2))/V(k+3); % diss I
CHKcar3(kt) =sum(dissC13v(k,n1+1:n2))/V(k+3);
ccp(k+6) = ccp(k+6) + sum(dissC13v(k,n2+1:Ns))/V(k+6); % diss D
CHKcar4(kt) =sum(dissC13v(k,n2+1:Ns))/V(k+6);
end;
if(ftys)
k=11;    
ccp(k  ) = ccp(k  ) + 2*Fin13*Aoc/V(k)/nOC         ... % Fin  L wthr:2
...
% % bio pump Corg
% for k=1:3
% cacp(k  ) = cacp(k )  -    ECLvCC(k)/V(k  );      % L 
% cacp(k+3) = cacp(k+3) + eI*EXLvCC(k)/V(k+3);      % I #!  EC or EP
% cacp(k+6) = cacp(k+6) + oI*EXLvCC(k)/V(k+6)...    % D #!(tot or Corg)
%                     + nu*EALvCC(k)/V(k+6)/2;    % D ClmnDiss
% end;    
% cacp(10)  = cacp(10)  -    ECHCC/V10;             % H
% for k=7:9                                       % DA,DI,DP
% cacp(k )  = cacp(k )  +    EXHCC/V(k)*gp(k)...    % #!  EC or EP
%                     + nu*EAHCC/V(k)*gp(k)/2;    %   
% end;                                            %   
% if(ftys)
% k = 11;
% cacp(k  ) = cacp(k  ) -    ECLvCC(4)/V(k  );
% cacp(k+1) = cacp(k+1) + eI*EXLvCC(4)/V(k+1);
% cacp(k+2) = cacp(k+2) + oI*EXLvCC(4)/V(k+2) ...
%                     + nu*EALvCC(4)/V(k+2)/2;  % D ClmnDiss
% end;
% % riverine & sediment fluxes
% if(fsed)                                        %   #!
% for k=1:3
% cacp(k  ) = cacp(k  ) + 1*Fin13*Aoc/V(k)/nOC         ... % Fin  L wthr:2
%                     + 1*FSi13*Aoc/V(k)/nOC         ... % #! Si
%                     - 1*Fkg  *Aoc/V(k)/nOC*alp*Rb(k);  % #! krgn
% cacp(k  ) = cacp(k  ) + sum(dissC13v(k,1   :n1))/V(k  ); % diss L
% cacp(k+3) = cacp(k+3) + sum(dissC13v(k,n1+1:n2))/V(k+3); % diss I
% cacp(k+6) = cacp(k+6) + sum(dissC13v(k,n2+1:Ns))/V(k+6); % diss D
% end;
% if(ftys)
% k=11;    
% cacp(k  ) = cacp(k  ) + 1*Fin13*Aoc/V(k)/nOC         ... % Fin  L wthr:2
%                     + 1*FSi13*Aoc/V(k)/nOC         ... % #! Si
%                     - 1*Fkg  *Aoc/V(k)/nOC*alp*Rb(k);  % #! krgn
% cacp(k  ) = cacp(k  ) + sum(dissC13v(4,1   :n1))/V(k  ); % diss L
% cacp(k+1) = cacp(k+1) + sum(dissC13v(4,n1+1:n2))/V(k+1); % diss I
% cacp(k+2) = cacp(k+2) + sum(dissC13v(4,n2+1:Ns))/V(k+2); % diss D
% end;    
% end;
%=================== 44CALCIUM  ======================%
% TH & mixing
...
CHKcarD1(kt)=1*Fin44*Aoc/V(k)/nOC          ...  % Fin  L
                 +1*FSi44*Aoc/V(k)/nOC;                 % #! Si
           
cacp(k  ) = cacp(k  )+1*sum(dissC44v(k,1   :n1))/V(k  ); % diss L
CHKcarD2(kt)=1*sum(dissC44v(k,1   :n1))/V(k  );
cacp(k+3) = cacp(k+3)+1*sum(dissC44v(k,n1+1:n2))/V(k+3); % diss I
CHKcarD3(kt)=1*sum(dissC44v(k,n1+1:n2))/V(k+3);
cacp(k+6) = cacp(k+6)+1*sum(dissC44v(k,n2+1:Ns))/V(k+6); % diss D
CHKcarD4(kt)=1*sum(dissC44v(k,n2+1:Ns))/V(k+6);
end;
if(ftys)
k=11;
cacp(k  ) = cacp(k  )+1*Fin44*Aoc/V(k)/nOC          ...  % Fin  L
...
% bio pump CaCO3, Aorg
for k=1:3
cacp(k  ) = cacp(k ) -      ECALv(k)/V(k  );%+ENLv(k)/V(k  ) ;
cacp(k+3) = cacp(k+3)+eI*(x*ECALv(k)/V(k+3));%-ENLv(k)/V(k+3));   % 1#!
cacp(k+6) = cacp(k+6)+oI*(x*ECALv(k)/V(k+6))...%-ENLv(k)/V(k+6))... % 1#!
                 +   nu*ECALv(k)/V(k+6);                   % D ClmnDiss
end;    
cacp(10)  = cacp(10)  - EAH/V10;%        + ENH/V10;
for k=7:9                                              % DA,DI,DP
cacp(k )  = cacp(k )  +(x*EAH/V(k))...%     - ENH/V(k))*gp(k)...% 1#!  
                  +nu*EAH/V(k)                *gp(k);  %   
end;
if(ftys)
k = 11;
cacp(k  ) = cacp(k  )-      ECALv(4)/V(k  );%+ENLv(4)/V(k  ) ;
cacp(k+1) = cacp(k+1)+eI*(x*ECALv(4)/V(k+1));%-ENLv(4)/V(k+1));
cacp(k+2) = cacp(k+2)+oI*(x*ECALv(4)/V(k+2))...%-ENLv(4)/V(k+2))... %
                 +   nu*ECALv(4)/V(k+2);
end;
% riverine & sediment fluxes
if(fsed)                                           % #!
for k=1:3
cacp(k  ) = cacp(k  )+1*Fin*Aoc/V(k)/nOC          ...  % Fin  L
                 +1*FSi*Aoc/V(k)/nOC;                 % #! Si
           
cacp(k  ) = cacp(k  )+1*sum(dissCv(k,1   :n1))/V(k  ); % diss L
cacp(k+3) = cacp(k+3)+1*sum(dissCv(k,n1+1:n2))/V(k+3); % diss I
cacp(k+6) = cacp(k+6)+1*sum(dissCv(k,n2+1:Ns))/V(k+6); % diss D
end;
if(ftys)
k=11;
cacp(k  ) = cacp(k  )+1*Fin*Aoc/V(k)/nOC          ...  % Fin  L
...
cacp(k  ) = cacp(k  )+1*sum(dissCv(4,1   :n1))/V(k  ); % diss L
cacp(k+1) = cacp(k+1)+1*sum(dissCv(4,n1+1:n2))/V(k+1); % diss I
cacp(k+2) = cacp(k+2)+1*sum(dissCv(4,n2+1:Ns))/V(k+2); % diss D
end;
end;
end;
...
yp(6*Nb+3+6*Ns:6*Nb+2+7*Ns) = f44cavAp; 
yp(6*Nb+3+7*Ns:6*Nb+2+8*Ns) = f44cavIp; 
yp(6*Nb+3+8*Ns:6*Nb+2+9*Ns) = f44cavPp;
if(ftys)
yp(6*Nb+3+9*Ns:6*Nb+2+10*Ns) = fcvTp; 
yp(6*Nb+3+10*Ns:6*Nb+2+11*Ns) = f13cvTp;   
yp(6*Nb+3+11*Ns:6*Nb+2+12*Ns) = f44cavTp;
end;    
end;
else % fdox
yp(     1:  Nb     ) = cp;
...
yp(5*Nb+3+6*Ns:5*Nb+2+7*Ns) = f44cavAp; 
yp(5*Nb+3+7*Ns:5*Nb+2+8*Ns) = f44cavIp; 
yp(5*Nb+3+8*Ns:5*Nb+2+9*Ns) = f44cavPp;
if(ftys)
yp(5*Nb+3+9*Ns:5*Nb+2+10*Ns) = fcvTp; 
yp(5*Nb+3+10*Ns:5*Nb+2+11*Ns) = f13cvTp;  
yp(5*Nb+3+11*Ns:5*Nb+2+12*Ns) = f44cavTp; 
end;    
end;
end; % fdox
else
