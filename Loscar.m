...
global Aoc rho Nb V A TH0 TH TS mv mhd kasv kkv TCv Sv Pv gp tA tI ...
    ...
    BlFlag CBl RBl kb FVC FVC13 Rvc pCSi nSi nCC Fkg Fkgs Fkg13 Rkg...
...
mv0   = [3.5+1 3.5+1 7.0+1 3.2 2.]*1e6;
...
fEPL  = 0.85;      % 0.80 0.9 LL utilization
...
pCSi  = 1000.*1.0;    % uatm, std-stt atm pCO2 560 1000 700 1000 /2
end; 
FVC   = 1*5.e12/Aoc;  % mol C, degassing /m2/y @280 uatm
nSi   = 0.2;          % 0.2 0.3
FVC   = 0.91*FVC*(2)^nSi; % initial 

%============== kerogen oxidation
Fkg   = 1*09.e12/Aoc;        % mol C    /m2/y 09
if(ftys)
Fkg   =   1.00*10.e12/Aoc;        % mol C    /m2/y 05
Fkgs  =   1*10.e12/Aoc;        % mol C    /m2/y 05
end
...
%(d13Cvc*5*(2)^nSi+d13Cin*(Finchck(end)/1e12)+d13Ckg*(10)-0*0-epsp*10)/(((Finchck(end)/1e12)+FSichck(end)/1e12)+10)
...
BUR57Ma=(sum(burialtA(1,:))+sum(burialtI(1,:))+sum(burialtP(1,:))+sum(burialtT(1,:)))/m2kg/1e6    %mol/my
BUR52Ma=(sum(burialtA(end,:))+sum(burialtI(end,:))+sum(burialtP(end,:))+sum(burialtT(end,:)))/m2kg/1e6 %mol/ny
...
axx = [-0.5e5 20e4];
sstr = '- ---.- ---.- ---. -: : : ';
% load dat\RlsCtv.dat;load dat\tv.dat ;load dat\tv11.dat ;load dat\c.DAT;...
%     load dat\a.DAT;load dat\a1.DAT;load dat\c1.DAT;load dat\co3tv1.DAT;...
%     load dat\pco2t1.DAT;load dat\Ca.DAT;load dat\CA.DAT ;load dat\CA1.DAT;...
%     load dat\pco2t.DAT ;load dat\ccdP.DAT;load dat\ccdT1.DAT;...
%     load dat\co3tv.DAT;load dat\FSichck.dat;load dat\Finchck.dat;...
%     load dat\FSichck1.dat ;load dat\Finchck1.dat;load dat\ccdP1.DAT;...
%     load dat\tvn.dat ;load dat\tv11n.dat;load dat\Can.DAT;load dat\CAn.DAT ;...
%     load dat\CA1n.DAT;load dat\FSichckn.dat;load dat\Finchckn.dat;...
%     load dat\FSichck1n.dat ;load dat\Finchck1n.dat;...
%     load dat\tv11s.dat; load dat\tvs.DAT; load dat\Cas1.dat;...
%     load dat\CAs.dat; load dat\CA1s.dat; load dat\ccdA.dat;...
%      load dat\ccdA1.dat; load dat\ccdI.DAT; load dat\ccdI1.DAT
Rm  =  (RlsCtv(1:end-1)+RlsCtv(2:end  ))/2./1.e15; % Gt C y
dtv =  (    tv(2:end  )-    tv(1:end-1));
tm  =  (    tv(1:end-1)+    tv(2:end  ))/2.;
dirstr='dat\';
save dat\fDTS.DAT DTS DTS2 ts3 DTS3 DTS4 -ASCII -DOUBLE -TABS;
% total release
TR = sum(Rm*diff(tv)*1e3);  % Gt C
% split release
DTSv = load([dirstr  'fDTS.DAT'])/1.e3;
%DTSv = load('fDTS.DAT')/1.e3;
...
% indices
[ax,kt1] = min(abs(tv-DTS     )); % end   1st bump
% [ax,kt3] = min(abs(tv-ts3     )); % start 2nd bump
% [ax,kt4] = min(abs(tv-ts3-DTS3)); % end   2nd bump
...
fill(trl,rl,'r ');
hold off;
%set(gca,'FontSize',fsr);
set(gca,'XLim',axx);
%set(gca,'YLim',[0 .25]);
% H=xlabel('XLb');
% set(H,'Vert','base');
set(gca,'FontSize',fs);
%set(gca,'Position',[0.13 0.8473 0.775 0.1],'XTickL',[]);
%ylabel('    Release (Pg C y^{-1})');
text(-40e3,0.500,' a)','FontSize',fs,'FontWeight','bold');
% text(005e3,0.400,['  \leftarrow',' 3,000 Pg C'],'FontSize',fs);
% text(50e3,0.100,['  \leftarrow','1,480 Pg C'],'FontSize',fs);
ylabel('Pg C y^{-1}');

%text(005,0.400,['  \leftarrow' str1],'FontSize',fs);
% % % if(size(dirstr) == size(dirstr23s))
% % % if(dirstr == dirstr23s)
% % % %text(020,0.200,['  \leftarrow' str3],'FontSize',fs);
% % % %text(080,0.045,['  \leftarrow' str2],'FontSize',fs);
% % % text(070,0.100,['  \leftarrow' str4],'FontSize',fs);
% % % %text(0.7,0.800,['Total = '     str ],'FontSize',fs,'Units','n');
% % % end;
% % % end;
text(0.1,0.100,['\delta^{13}C  = ' dstr '‰'],'FontSize',fs,'Units','n');
%text(0.7,0.600,['\delta^{13}C ]''''[' char(8240)']','FontSize',fs,'Units','n');

set(gca,'YLim',[0 .1],'XLim',[0 5e6])


figure(101)
DT=(log(max(pco2t)/pco2t(1))/log(2))*3.0
d13c57ma=(d13c(1,1)*V(1)+d13c(1,2)*V(2)+d13c(1,3)*V(3)+d13c(1,11)*V(11))/((V(1)+V(2)+V(3)+V(11)))
d13c52ma=(d13c(end,1)*V(1)+d13c(end,2)*V(2)+d13c(end,3)*V(3)+d13c(end,11)*V(11))/((V(1)+V(2)+V(3)+V(11)))
d13c57madeep=(d13c(1,7)*V(7)+d13c(1,8)*V(8)+d13c(1,9)*V(9)+d13c(1,13)*V(13))/((V(7)+V(8)+V(9)+V(13)))
d13c52madeep=(d13c(end,7)*V(7)+d13c(end,8)*V(8)+d13c(end,9)*V(9)+d13c(end,13)*V(13))/((V(7)+V(8)+V(9)+V(13)))
DCCDa=ccdA(1)-ccdA(end);
DCCDp=ccdP(1)-ccdP(end);
%dstr = sprintf(date);
dstr1 = sprintf('%+3.1f',DT);
dstr2=sprintf('%+3.2f',d13c52ma-d13c57ma);
dstr3=sprintf('%+3.2f',d13c52madeep-d13c57madeep);
dstrA=sprintf('%+3.0f',-DCCDa);
dstrP=sprintf('%+3.0f',-DCCDp);
subplot(511)
plot(trl,rl,'-k');
hold on;
fill(trl,rl,'r ');
...
text(0.1,0.2,['\delta^{13}C  = ' dstr '‰'],'FontSize',fs,'Units','n');
set(gca,'XTickLabel',[])
subplot(512)
plot((57*1e6-tv)/1e6  ,pco2t ,'b-','LineWidth',2);
Ylim([400 2000])
set(gca,'XTickLabel',[],'XDir','reverse')
ylabel('pCO_2 (ppmv)')
text(56,1300,['\Delta{T}  = ' dstr1 '^oC'])
text(57.,1750,' b)','FontSize',fs,'FontWeight','bold');
%title('LosCARB')
% hold on
% [a,b]=max(pco2t);
% plot((57*1e6-tv(b))/1e6,a,'pr')
% hold off

subplot(513)
plot((57*1e6-tv)/1e6, FSichck/1e12,'m',(57*1e6-tv)/1e6,Finchck/1e12,'k');%(57*1e6-tv)/1e6,Fkgchck/1e12,'g',(57*1e6-tv)/1e6,Fkgschck/1e12,'r')
ylim([4 25])
ylabel('Fluxes (10^{12} mol yr^{-1})')
text(57.,23.5,' c)','FontSize',fs,'FontWeight','bold');
%ylabel('FSi,FC,Fkg');
legend('Fws','Fwc');%,'Fbg','Fwg')
set(gca,'XTickLabel',[],'XDir','reverse')
...
%plot((XX(1737:2231,1)*(-1)+57)*1e6,XX(1737:2231,2))

plot((57*1e6-tv)/1e6  ,d13c (:,1),'g',(57*1e6-tv)/1e6  ,d13c (:,3),'k',(57*1e6-tv)/1e6  ,d13c (:,7),'r','LineWidth',2);
...
ylabel('\delta^{13}C (‰)');
legend('LA','LP','DA')
set(gca,'YDir','reverse','XTickLabel',[],'XDir','reverse')
text(56,5,[' \Delta\delta^{13}C_{surf}=' dstr2 '‰']);
text(56,1,[' \Delta\delta^{13}C_{deep}=' dstr3 '‰']);
text(57.,0.5,' d)','FontSize',fs,'FontWeight','bold');
subplot(515)
plot((57*1e6-tv)/1e6,ccdA,'g',(57*1e6-tv)/1e6,ccdP,'k','LineWidth',2)
legend('Atlantic','Pacific')
text(57,5200,[' \Delta CCD=' dstrA '(m)'],'Color','g');
text(57,3500,[' \Delta CCD=' dstrP '(m)']);
text(57,2300,' e)','FontSize',fs,'FontWeight','bold');
Ylim([2000 6000])
ylabel('CCD (m)')
...
% plot((57*1e6-tv(b))/1e6,a,'pr')
% hold off
%set(gca,'XTickLabel',{'57','','56','','55','','54','','53','','52','51','','50','','49'});
xlabel('Age (Ma)')
end; % sediments
